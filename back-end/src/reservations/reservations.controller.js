...
//lists reservations based off mobile number
async function list(req, res) {
	const {date} = req.query;
	const {mobile_number} = req.query;
	const reservations = await service.list(date, mobile_number);
	const filteredData = reservations.filter((reservation) => reservation.status !== "finished");

	res.json({ data: filteredData });
}

//verifies data object exists
...
//verifies time and date are aligned with business hours of restaurant
...
// validates, searches and stores reservation beased off of id
